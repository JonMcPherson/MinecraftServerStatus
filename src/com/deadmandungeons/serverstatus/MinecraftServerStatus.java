...
public class MinecraftServerStatus {
	...
	private static Address resolveAddress(String host, int port) throws URISyntaxException {
		try {
			return lookupAddress(host, port);
		} catch (Exception e) {
	...
	private static Address resolveAddress(Address address) {
		try {
			return lookupAddress(address.getHost(), address.getPort());
		} catch (Exception e) {
			return address;
		}
	}
	
	private static Address lookupAddress(String host, int port) throws Exception {
		...
		port = Integer.parseInt(answer[answer.length - 2]);
